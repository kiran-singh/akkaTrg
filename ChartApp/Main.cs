...
namespace ChartApp
{
    public partial class Main : Form
    {
        ...
        private Dictionary<Metric.CounterType, IActorRef> _toggleActors = new();
        ...
        private void Main_Load(object sender, EventArgs e)
        {
            ...
            _toggleActors[Metric.CounterType.Cpu] = Program.ChartActors.ActorOf(
                Props.Create(() => new ButtonToggleActor(_coordinatorActor, btnCpu, 
                        Metric.CounterType.Cpu, false))
                    .WithDispatcher("akka.actor.synchronized-dispatcher"));

            // MEMORY button toggle actor
            _toggleActors[Metric.CounterType.Memory] = Program.ChartActors.ActorOf(
                Props.Create(() => new ButtonToggleActor(_coordinatorActor, btnMemory,
                        Metric.CounterType.Memory, false))
                    .WithDispatcher("akka.actor.synchronized-dispatcher"));

            // DISK button toggle actor
            _toggleActors[Metric.CounterType.Disk] = Program.ChartActors.ActorOf(
                Props.Create(() => new ButtonToggleActor(_coordinatorActor, btnDisk,
                        Metric.CounterType.Disk, false))
                    .WithDispatcher("akka.actor.synchronized-dispatcher"));

            // Set the CPU toggle to ON so we start getting some data
            _toggleActors[Metric.CounterType.Cpu].Tell(new ButtonToggleActor.Toggle());
        }
        ...
        {
            _toggleActors[Metric.CounterType.Memory].Tell(new ButtonToggleActor.Toggle());
        }

        private void btnDisk_Click(object sender, EventArgs e)
        {
            _toggleActors[Metric.CounterType.Disk].Tell(new ButtonToggleActor.Toggle());
        }
